...
async function displayMatchHistoryHead(gameMode) {
	const tableHeaders = {
		...
		"pac-pong": ["Date", "Left", "Right", "Pacman", "Score (L-R-P)"],
...
async function genPacPongTableBody(games, matchHistoryBody) {
	games.forEach(game => {
		...
		// Extract player usernames and scores
		const leftPlayer = Object.keys(game.score.left)[0];
		const leftScore = Object.values(game.score.left)[0];

		const rightPlayer = Object.keys(game.score.right)[0];
		const rightScore = Object.values(game.score.right)[0];

		const pacPlayer = Object.keys(game.score.pac)[0];
		const pacScore = Object.values(game.score.pac)[0];

		// Create table cells
		const leftCell = document.createElement('td');
		leftCell.textContent = leftPlayer;

		const rightCell = document.createElement('td');
		rightCell.textContent = rightPlayer;

		const pacCell = document.createElement('td');
		pacCell.textContent = pacPlayer;

		const scoreCell = document.createElement('td');
		scoreCell.textContent = `${leftScore}-${rightScore}-${pacScore}`;

		// Determine the result for the current user
		const username = document.getElementById('username').textContent;
		const resultCell = document.createElement('td');

		// if (leftPlayer === username) {
		// 	resultCell.textContent = getGameStatus([leftScore, rightScore, pacScore], 0);
		// } else if (rightPlayer === username) {
		// 	resultCell.textContent = getGameStatus([leftScore, rightScore, pacScore], 1);
		// } else if (pacPlayer === username) {
		// 	resultCell.textContent = getGameStatus([leftScore, rightScore, pacScore], 2);
		// } else {
		// 	console.error(`Game with id ${game.id} can't be connected to current user: ${username}`);
		// 	return;
		// }
